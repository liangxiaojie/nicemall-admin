...
<script>
import { getGallerys, createGallery, updateGallery, deleteGallery } from '@/apollo/gallery';
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createGallery(this.$apollo, this.temp).then((data) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateGallery(this.$apollo, tempData).then(data => {
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row) {
      this.$confirm('确认删除?', '提示', {
          ...
          deleteGallery(this.$apollo, row._id).then(data => {
            this.$notify({
              title: '成功',
              message: '删除成功',
              type: 'success',
